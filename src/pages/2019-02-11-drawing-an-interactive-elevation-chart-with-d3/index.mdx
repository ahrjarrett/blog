...
Next we need to draw some markers and connect them by a path (or “polyline”, in Google-Maps-speak). We can’t use the Directions API without a Premium account, and last I checked it takes \$10K to open one of those. So our polyline will be an approximation of a user path.

In our app, we made all of this data come from user interaction. Dropping, dragging and deleting markers and changing map state is not the focus of this post, and I’m sure you already have ideas about how you would go about doing that.

We’re going to keep hardcoding values, but we’ll switch it up to keep things lively. Let’s go to San Francisco, because I can’t afford to go in real life anymore:
...
Great! We have 18 markers going down Lombard Street. Notice how clicking show adjusts the map’s zoom and bounds to neatly frame the markers, while hiding them returns our map to its initial state.

Here’s the method that builds up a `LatLngBounds` object for us and then adjusts our maps “lens” to contain it:
...
If you’ve worked with the GM API for any length of time, you’ve probably gotten tired of keeping track of whether you’re dealing with an instance of the LatLng class — where latitude, for example, is accessible by calling the function `LatLng.position.lat()`.

We got tired of converting back and forth, so `setMapBounds` accepts both. For every coordinate object or LatLng instance we pass in, we adjust the outer bounds of our map, and we fit the map to those boundaries.
...
Notice that we did not make this method as flexible w/r/t its arguments as setMapBounds; we found drawing and especially redrawing polylines to be slightly more finicky, and did not feel like it needed _more_ flexibility as much as it needed proper documentation of its function signature.

We add the polyline to our class for use throughout the rest of the component. We also need to keep track of it so we can remove it later when the user clicks “Hide Path” (“Hide Markers” too, otherwise we’d have a marker-less path floating around with nothing anchoring it down!).

Finally, we’ll get our elevation samples.

### Map Showing the Elevation Delta Between Points

<MapWithMarkers
  exampleType="elevationInfobox"
  center={mapCenterHI}
