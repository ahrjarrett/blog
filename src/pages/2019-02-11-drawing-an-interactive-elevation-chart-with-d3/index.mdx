...
**Second**, in lines 49-61 (highlighted) we call the Elevations API, passing in the path we want to take samples from along with the number of samples we want. We ask for 100 samples because again, math is hard (also I like the way these zeroes look: **0**).
